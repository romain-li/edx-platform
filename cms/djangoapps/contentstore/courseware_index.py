...
class CourseAboutSearchIndexer(object):
    ...
    @classmethod
    def remove_deleted_items(self, structure_key):
        ...
        response = searcher.search(
            doc_type=self.DISCOVERY_DOCUMENT_TYPE,
            field_dictionary=self._get_location_info(structure_key)
        )
        result_ids = [result["data"]["id"] for result in response["results"]]
        searcher.remove(self.DISCOVERY_DOCUMENT_TYPE, result_ids)
