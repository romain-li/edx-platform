...
class TestCoursewareSearchIndexer(MixedWithOptionsTestCase):
    ...
    def _test_delete_course_from_search_index_after_course_deletion(self, store):
        """
        Test that course will also be delete from search_index after course deletion.
        """
        # this will only fetch courses about information used in /course_discovery_search
        self.DOCUMENT_TYPE = 'course_info'
        response = self.search()
        self.assertEqual(response["total"], 0)
